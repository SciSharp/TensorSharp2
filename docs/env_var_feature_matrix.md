...
| `TS_GGML_TP_AR_PROBE` | local TP, `ggml_cuda` | `0` skips the NCCL pre-flight probe; `force` re-probes, ignoring the cached verdict (`~/.cache/tensorsharp/tp-collective-probe`). The probe runs one small AllReduce end to end before model load — some cloud hosts advertise P2P that never delivers, and NCCL's first collective then spins both GPUs forever | probe on, verdict cached per driver/NCCL/GPU set | not registered | no |
| `TS_GGML_TP_AR_PROBE_MS` | local TP, `ggml_cuda` | Deadline for the probe's AllReduce to complete before the collective is declared broken and rerouted to the pinned-host `internal` pipeline; `0` disables the probe | `10000` ms | not registered | no |
