...
Speculative decoding is **off by default**. Enable it with `--mtp-spec` (env `TS_MTP_SPEC=1`) on the server or on `TensorSharp.Cli`:
...
Tuning: `--mtp-draft` (default `8`) bounds tokens drafted per step; `--mtp-pmin` is the minimum draft-head confidence to keep a token, and drafting stops at the first token below it. Left unset, the gate comes from the drafter kind: `0.75` for a per-token head, `0.35` cumulative for a block drafter. The two knobs interact — a wide window occasionally forms a long chain that is mostly rejected, and those verify rows are paid for either way — so they are worth sweeping together on a new model/host pair. On GLM 5.2, `--mtp-draft 4 --mtp-pmin 0.55` was best or tied-best in every measured run, by ~4% over the defaults. Gemma 4 draft-path A/B switches are the `TS_GMTP_*` env vars (see the **MTP / speculative-decoding tunables** table under [Web Application](USAGE.md#web-application)). Per-architecture mechanics are in the [Qwen 3.5/3.6 card](docs/models/qwen35.md), the [GLM card](docs/models/glm.md#nextn--mtp-speculative-decoding) and the [Gemma 4 card](docs/models/gemma4.md).
