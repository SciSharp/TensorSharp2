...
- **GLM 5.x:** uses `<think>...</think>` tags and is the other way round -- the `Reasoning Effort` system line makes thinking the default, and the prompt opens the block for the model to close; with thinking off the prompt emits an empty `<think></think>` so the model answers directly
