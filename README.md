...
| **GLM 5.2** | The server's `--mtp-spec` — nothing else to download, the NextN block is already in the checkpoint | Decode **1.27× median over five runs** (range 1.14–1.40×) on 2× RTX PRO 6000 with `--n-cpu-moe 20`, at 94% draft acceptance; `--mtp-draft 4 --mtp-pmin 0.55` was worth another ~4% in every run. Costs ~3 GiB of VRAM for the extra block, so it is only paged in when the flag is set. |
