...
- **🧠 GLM-5.2 (744B-A40B MoE) with tensor parallelism and CPU MoE offload.** Multi-head Latent Attention plus a DeepSeek Sparse Attention "lightning indexer" that picks which 2048 cached tokens each query may attend to. `--tp N` runs every layer on every GPU (heads column/row-parallel, every expert split row-wise), `--cpu-moe` keeps the routed experts — 92% of the checkpoint — in system RAM, and both reproduce llama.cpp token-for-token. → [GLM card](docs/models/glm.md)
...
| GLM 5.x | `glm-dsa` | GLM-5.2 (744B-A40B MoE, 256 experts, MLA + DeepSeek Sparse Attention, 1M context) | Text only | Yes | Yes (XML tool calls) | — | [glm.md](docs/models/glm.md) |
